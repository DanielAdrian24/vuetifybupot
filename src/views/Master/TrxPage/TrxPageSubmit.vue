...
<v-container class="my-5">
  ...
  >
    <template v-slot:[`item.bupot_number`]="{ item }">
        ...
        <a @click="showDialog(item)" v-if="item.status === 'D'" class="font-weight-bold">
          {{ item.bupot_number }}
        </a>                     
        <!-- </router-link> -->
    </template>
...
import moment from 'moment'
  export default {
    ...
    methods:{
        updateSubmit () {
            this.idBupot.splice(0);
            this.selected.forEach(item => {
                this.idBupot.push(item.bupot_id)
            })
            this.$swal.fire({
              ...
              if (result.isConfirmed) {
                axios({
                    method: 'post',
                    url: 'http://localhost:8000/api/updatestatusbupotsubmit',
                    data: {
                      id_bupot: this.idBupot
                    },
                  })
                   .then(response => {
                      this.$swal.fire(
        ...
        searchData(){
            axios({
                method: 'post',
                url: 'http://localhost:8000/api/searchdatadraft',
                data: {
